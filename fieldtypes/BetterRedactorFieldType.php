...
class BetterRedactorFieldType extends RichTextFieldType
{
	...
	private function _getConfigJson()
	{
		...
		if (empty($json))
		{
            $pluginFolder = mb_strtolower($this->getClassHandle());
            $defaultJson = craft()->path->getPluginsPath() . $pluginFolder . '/config.json';
            $json = IOHelper::getFileContents($defaultJson);
		}
	...
	private function _includeFieldResources()
	{
        ...
        craft()->templates->includeCssFile('//maxcdn.bootstrapcdn.com/font-awesome/4.6.3/css/font-awesome.min.css');

        // Include builtin plugins
        $pluginsDir = craft()->path->getResourcesPath() . 'lib/redactor/plugins';
        if (file_exists($pluginsDir)) {
            $files = array_filter(
                scandir($pluginsDir),
                function($file) use ($pluginsDir) {
                    return is_file("$pluginsDir/$file");
                }
            );

            foreach ($files as $file) {
                if (preg_match('(.js$)i', $file)) {
                    craft()->templates->includeJsResource("lib/redactor/plugins/" . $file);
                } elseif (preg_match('(.css$)i', $file)) {
                    craft()->templates->includeCssResource("lib/redactor/plugins/" . $file);
                }
            }
        }
